...
| 派发次数超过预算 | **调用被拒** |
| 写了不属于自己的文件 | **调用被拒** |
| 同时运行的 Worker 超过上限 | **调用被拒** |
| 有 Worker 启动后**再无音讯** | **本轮收尾被拦住** |
...
| 字段 | 管什么 | 默认 |
| --- | --- | --- |
| `spawnBudget` | 一个 campaign 总共能派发多少次 | 16 |
| **`maxParallel`** | **同时最多几个在跑** | **4** |

**为什么需要第二个**：总预算管的是总额，管不住形状。允许 16 次派发，不等于允许**一次起 16 个**。

> 一次真实事故：8 个 Worker 并行，上游在 20:19:47 一秒掐断全部 8 条在途请求，6 个再没回来。
> 总预算是 16，完全允许那次运行再次发生。

**并发数在批准时由你定**，因为那是你的取舍：跑得快，和断线时一次损失多少。

> ⚠️ **僵尸派发检查**：Worker 启动后如果再无音讯，门禁会在收尾时拦住并点名。
> 这是那次事故里**本该有却没人看的信号**——6 个 Worker 静默死亡，而所有面板都是绿的。
>
> **边界**：它分得清「启动了但没回音」和「启动了已完成」，**分不清「启动了正在跑」**——
> 正在跑的 Worker 也持有预留。所以只在收尾时检查，那时不该还有东西在跑。
...
| 并发上限 | `campaign.json: {"maxParallel": 0}`（0 = 不限） |
| 僵尸派发检查 | `campaign.json: {"requireDispatchesSettled": false}` |
...
tests/                     # 921 项：结构校验 + 钩子行为 + 计划库 + 引擎 + 路径与隔离 + 证据
docs/机制说明.md            # 深入机制
