...
> 804 项测试 + 四轮真机验证；**7 个真机缺陷全部修复**（详见 CHANGELOG 0.3.1）。
>
...
3. **这件事值得花比平常多 10~100 倍的 token 吗？**
...
**特别注意：别让它并发写相邻文件。** 平台的子代理载荷里没有 per-subagent 标识，
同一会话里的多个 Worker 会解析成同一个 owner，文件独占**不是弱化，是不存在**。
要并发就串行派 Worker，一次一个。
